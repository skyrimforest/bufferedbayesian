...
class BayesianOptimization(Observable):
    ...
    def __init__(self,
        ...
        # 在记录的样本点达到self._exp_buffer.threshold1后,每隔max_size*20%个样本点到来,会重置GP模型
        ...
        if constraint is None:
            ...
            self._space = TargetSpace(f, pbounds, random_state=random_state,
                                      allow_duplicate_points=self._allow_duplicate_points,exp_buffer=exp_buffer)
    ...
    def register(self, params, target, constraint_value=None):
        ...
        """
        # space进行一个register,每次输入的是一个采样点和目标值
    ...
    def suggest(self, utility_function):
        ...
        with warnings.catch_warnings():
            ...
            # 这里使用space中的所有参数去fit这个高斯模型.
            # 居然是所有参数,那事情就好办了,只需要对space下手就可以了.
            # 这样就不需要进行每隔多少点更新模型了,因为它很自然地每次都会更新模型.
            # print(f"看这里是参数:{self._space.params}")
            # print(f"看这里是目标值:{self._space.target}")
