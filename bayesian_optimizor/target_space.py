...
class TargetSpace():
    ...
    def __init__(self, target_func, pbounds, constraint=None, random_state=None,
                 allow_duplicate_points=False,exp_buffer=None):
    ...
    def register(self, params, target, constraint_value=None):
        ...
        # 这里是自带的解空间。
        # 需要对它进行手动管理
        # 优化器register时,直接调用本函数
        # suggest时,会使用全部的params和target.
        # 因此只需要完全地按照缓冲区的思路管理params和target就行了
        # 这样,之后suggest的时候用的就是新的params
        ...
        # 使用缓冲池管理
        # 实际上相当于在现在的_params和_target与外界之间加了一层.
        new_x=np.concatenate([x,[target]])
        # new_x是这样的,最后一个位置为target,前面为旧的x.
        self._exp_buffer.add_point(list(new_x))
        result=self._exp_buffer.get_real_exp()
        new_param=np.array([row[:-1] for row in result])
        new_target=np.array([row[-1] for row in result])
